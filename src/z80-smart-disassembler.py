...
def arrayAsDB(a):
    ...
    for i in range(len(a)):
        ...
        if a[i][0] == '#':
            ...
            v = int(a[i][0:])
        res += hx(v)  # a[i]
        letter = v & 0xdf  # Remove bit 5
        if letter >= 65 and letter < 65+26:
            nc += 1
            if v & 32 == 32:
                st += string.ascii_lowercase[letter-65]
            else:
                st += string.ascii_uppercase[letter-65]
        else:
            st = st+'.'
    if nc > 0:
        res += ' ;'+st
...
ap.add_argument('-o', '--output-prefix',
                help='Prefix used for generating output files')
ap.add_argument('-O', '--org',
                default="0",
                help='Adress of the beginning of the bin file in RAM')
ap.add_argument('-p', '--disark-path',
                default='',
                help='Path to Disark executable')
ap.add_argument('-d', '--use-disark',
                action='store_true',
                help='Use Disark for disassembling')
ap.add_argument('-a', '--start-adresses',
                nargs="+",
                help='Start adresses (in hex format)')
ap.add_argument('-s', '--symbols',
                help='Additional symbol file to use for disassembling with '
                'disark.')
ap.add_argument('-r', '--regions',
                help='Region file to use for disassembling with disark.')
ap.add_argument('-x', '--exclude-adresses',
                nargs="+",
                help='Exclude adresses (in hex format) - code won\'t be '
                'disassembled')
ap.add_argument('-z', '--zero',
                nargs="+",
                help='Region to fill with zeroes. (addr1-addr2 or addr1+nbytes'
                ', in hexadecimal)')
ap.add_argument('-v', '--verbose',
                default=0,
                action='count',
                help='Increase Verbosity')
ap.add_argument('-G', '--dot',
                action='store_true',
                help='Generates a dot (graphviz) file')
ap.add_argument('-c', '--check',
                action='store_true',
                help='Reassemble and diff')
ap.add_argument('-R', '--rasm',
                default='rasm',
                help='Path to rasm binary')
ap.add_argument('-u', '--undocumentedOpcodes',
                action='store_false',
                help='Undocumented Opcodes to bytes')
...
# Exclude
if args['exclude_adresses'] is not None:
...
while len(pcstack) > 0:
    ...
    while True:
        ...
        if opcode == 'jp' or opcode == 'jr' or ('rst' in opcode):
            ...
            if ',' in op[1]:
                ii = op[1].index(',')+1
                # print (hx(start_pc), hx(pc), opcode, data, sz, op[1][ii:],
                #        hx(int(op[1][ii:], 16)))
                addJump(pc,
                        int(op[1][ii:], 16),
                        opcode,
...
if args['regions'] is None:
    ...
    while i < len(mem):
        ...
        if t != curZoneCodeType:
            ...
            # print(';Zone', curZoneCodeType, hx(curZoneStart),
            #       hx(curZoneEnd-curZoneStart))
            if curZoneCodeType is False:
                symfile.write('data'+format(curZoneStart, '04X') +
                              ' '+hx(curZoneStart)+'\n')
                symfile.write('DisarkByteRegionStart'+hx(curZoneStart)+' ' +
                              hx(curZoneStart)+'\n')
                symfile.write('DisarkByteRegionEnd'+hx(curZoneStart)+' ' +
                              hx(curZoneEnd+1)+'\n')
            curZoneStart = i
            curZoneCodeType = t
...
if args['use_disark']:
    ...
    options = [path,
               ...
               "--genLabels",
               "--src8bitsValuesInHex",
               "--src16bitsValuesInHex",
               "--symbolFile",
               symfilename]
    if args['undocumentedOpcodes'] is False:
    ...
    # TODO: handle dw
    for letter in dfileContent:
        ll = letter.strip()
        # starts by a label?
        if ('start' in ll and ll.index('start') == 0) or \
           ('data' in ll and ll.index('data') == 0) or \
           ('lab' in ll and ll.index('lab') == 0):
            if len(dbl) > 0:
...
if args['dot']:
    ...
    for j in jplist:
        ...
        dotfile.write('lab'+format(i[0], '04x') +
                      ' -> '+'lab'+format(i[1], '04x')+' '+options+';\n')
            ...
            if k in jplist:
                ...
                if k in jptolist:
                    ...
                    if nxt in nextopcode:
                        ...
                        dotfile.write('lab'+format(k0, '04x')+' -> ' +
                                      'lab'+format(k1, '02x') +
                                      '[style=dotted] ;\n')
                        nodeAttributes[k0] = 1
                        nodeAttributes[k1] = 1
    ...
    for k in nodeAttributes:
        dotfile.write('lab'+format(k, '04x') + ' [' +
                      'label="#'+format(k, '04x')+'"'+']'+';\n')
